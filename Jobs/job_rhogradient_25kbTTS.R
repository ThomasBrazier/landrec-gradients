...
downstream = 25000 # Max size of the downstream region
...
genes = genes[unlist(pbmclapply(genes, function(x) {"CDS" %in% x$feature}, mc.cores = ncores))]
...
gene_tts = pbmclapply(genes, function(x){if (sum(x$feature == "CDS" & x$rank == 1) > 0) {ifelse("+" %in% x$strand,
...
make.range = function(x) {
  # Make a -upsteeam:+downstream interval in which to sample windows
  # Tak an index of an atg as argument
  if ("+" == unique(as.character(strand[x]))) {
    start = as.numeric(gene_tts[x]) + as.numeric(idx[[x]])
    range = GRanges(seqnames = as.character(seqname[x]),
                    ...
                    gene_tts = as.character(gene_tts[x]))
  } else {
    # TODO take end instead of start for ATG
    start = as.numeric(gene_tts[x]) - abs(interval) + 1 - as.numeric(idx[[x]])
    end = as.numeric(gene_tts[x]) - as.numeric(idx[[x]])
    range = GRanges(seqnames = as.character(seqname[x]),
...
# gene_ranges = lapply(1:length(gene_tts), function(x){make.range(x)})
# for (i in 1:100) {
#   make.range(i)
# }
gene_ranges = pbmclapply(1:length(gene_tts), function(x){make.range(x)}, mc.cores = ncores)
...
if (chromosome == "all") {
  save(df_pos, file = paste("Output/RhoGradient_25kbTTS_", dataset, ".Rda", sep = ""))
} else {
  save(df_pos, file = paste("Output/RhoGradient_25kbTTS_", dataset, "_", chromosome, ".Rda", sep = ""))
}

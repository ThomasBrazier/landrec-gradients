...
interval = 500
upstream = -25000 # Size of the upstream region
genic = 25000 # Max size of the genic region
...
seqname = pbmclapply(genes, function(x){unique(x$seqname)}, mc.cores = ncores)
...
p.sample = 0.7 # Gene proportion to sample
# in order to avoid effects of gene end

# Get size of each gene
gene_size = unlist(lapply(genes, function(x){x$end[x$feature == "gene"] - x$start[x$feature == "gene"] + 1}))
# Calculate size to cut
gene_size = gene_size * p.sample
gene_size = unlist(lapply(gene_size, function(x) {min(2000, x)}))
...
make.range = function(x) {
  ...
  if ("+" == unique(as.character(strand[x]))) {
    start = as.numeric(gene_atg[x]) + as.numeric(idx[[x]])
    range = GRanges(seqnames = as.character(seqname[x]),
            ...
            gene_atg = as.character(gene_atg[x]), dist_tss = dist_tss[x])
  } else {
    # TODO take end instead of start for ATG
    start = as.numeric(gene_atg[x]) - abs(interval) + 1 - as.numeric(idx[[x]])
    end = as.numeric(gene_atg[x]) - as.numeric(idx[[x]])
    range = GRanges(seqnames = as.character(seqname[x]),
...
gene_ranges = pbmclapply(1:length(gene_atg), function(x){make.range(x)}, mc.cores = ncores)
...
hits = findOverlaps(res, ldmap_ranges)
hits2 = split(hits, as.factor(queryHits(hits)))
mean.rho.control = unlist(pbmclapply(1:length(res), function(x) {if (x %in% names(hits2)) {if (length(subjectHits(hits2[[which(names(hits2) == x)]])) > 0) {mean(ldmap_ranges$Mean_rho[subjectHits(hits2[[which(names(hits2) == x)]])], na.rm = TRUE)} else {NA}} else {NA}}, mc.cores = ncores))
...
if (chromosome == "all") {
  save(df_pos, file = paste("Output/RhoGradient_25kbATG_", dataset, ".Rda", sep = ""))
} else {
  save(df_pos, file = paste("Output/RhoGradient_25kbATG_", dataset, "_", chromosome, ".Rda", sep = ""))
}

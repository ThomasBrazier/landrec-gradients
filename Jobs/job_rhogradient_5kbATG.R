...
# ids = ids[1:100]
cat("....... Sample genes\n")
genes = pbmclapply(ids, function(x){sample.gene(x)},
                   mc.cores = ncores)
# genes = pbmclapply(1:length(ids), function(x){sample.gene(as.character(ids[[x]]))})
# DONE Bugfix not all parts of the gene are sampled

# Keep only genes with CDS
genes = genes[unlist(pbmclapply(genes, function(x) {"CDS" %in% x$feature},
                                mc.cores = ncores))]

# For each gene, get a range and a random control ----

cat("Get position of first ATG\n")
# Get position of first ATG ----
# TODO BUGFIX problem of wrong direction for strand "-"
# TODO take end instead of start for ATG
gene_atg = pbmclapply(genes, function(x){if (sum(x$feature == "CDS" & x$rank == 1) > 0) {ifelse("+" %in% x$strand,
                                                                           min(x$start[which(x$feature == "CDS" & x$rank == 1)]),
                                                                           max(x$end[which(x$feature == "CDS" & x$rank == 1)]))}
  else {NA}}, mc.cores = ncores)
strand = pbmclapply(genes, function(x){unique(as.character(x$strand))},
                    mc.cores = ncores)
seqname = pbmclapply(genes, function(x){unique(x$seqname)},
                     mc.cores = ncores)

cat("Get distance to TSS\n")
# Get distance to TSS ----
tss = pbmclapply(genes, function(x){if (sum(x$feature == "CDS" & x$rank == 1) > 0) {ifelse("+" %in% x$strand,
                                                                                                min(x$start[which(x$feature == "gene" | x$feature == "mRNA")]),
                                                                                                max(x$end[which(x$feature == "gene" | x$feature == "mRNA")]))}
  else {NA}}, mc.cores = ncores)

dist_tss = abs(unlist(gene_atg) - unlist(tss))
...
# Same range, cut in further analyses
# idx = lapply(genes, function(x) {seq(from = upstream, to = downstream, by = abs(interval))})

# # One specific range per gene
# idx = lapply(gene_size, function(x) {seq(from = upstream, to = x, by = abs(interval))})

make.range = function(x) {
  # Make a -upstream:+downstream interval in which to sample windows
  # Take an index of an atg as argument
  if ("+" == unique(as.character(strand[x]))) {
    start = as.numeric(gene_atg[x]) + as.numeric(idx)
    range = GRanges(seqnames = as.character(seqname[x]),
            ...
            gene_atg = as.character(gene_atg[x]),
            dist_tss = dist_tss[x])
  } else {
    # TODO take end instead of start for ATG
    start = as.numeric(gene_atg[x]) - abs(interval) + 1 - as.numeric(idx)
    end = as.numeric(gene_atg[x]) - as.numeric(idx)
    range = GRanges(seqnames = as.character(seqname[x]),
...
# gene_ranges = lapply(1:length(gene_atg), function(x){make.range(x)})
gene_ranges = pbmclapply(1:length(gene_atg),
...
mean.rho = unlist(pbmclapply(1:length(res),
...
hits = findOverlaps(res, ldmap_ranges)
hits2 = split(hits, as.factor(queryHits(hits)))
mean.rho.control = unlist(pbmclapply(1:length(res),
                                     function(x) {if (x %in% names(hits2)) {if (length(subjectHits(hits2[[which(names(hits2) == x)]])) > 0) {mean(ldmap_ranges$Mean_rho[subjectHits(hits2[[which(names(hits2) == x)]])], na.rm = TRUE)} else {NA}} else {NA}},
                                     mc.cores = ncores))
...
gene_ranges$gene_upstream_overlap = (gene_ranges$gene_overlap & (gene_ranges$idx < (-gene_ranges$dist_tss - 100)))
...
# exon_boundaries = makeGRangesFromDataFrame(gff[which(gff$feature == 'CDS'),],
#                                            keep.extra.columns = T)
# exon_boundaries = reduce(exon_boundaries)
# hits = findOverlaps(gene_ranges, exon_boundaries)
# overlaps = pintersect(gene_ranges[queryHits(hits)], exon_boundaries[subjectHits(hits)])
# percentOverlap = width(overlaps) / width(exon_boundaries[subjectHits(hits)])
# df = data.frame(hits = queryHits(hits),
#                 overlap = percentOverlap)
# 
# gene_ranges$exon_overlap[queryHits(hits)] = width(overlaps) / width(exon_boundaries[subjectHits(hits)])
# 
# gene_ranges$exon_overlap[queryHits(hits)] =
#   
# # ex_over = pbmclapply(1:length(gene_ranges), function(x) {size_overlap(gene_ranges[x], exon_boundaries)})
# # gene_ranges$exon_overlap = unlist(ex_over)
# 
# cat("Intron overlap\n")
# intron_boundaries = makeGRangesFromDataFrame(gff[which(gff$feature == 'intron'),])
# # int_over = pbmclapply(1:length(gene_ranges), function(x) {size_overlap(gene_ranges[x], intron_boundaries)})
# # gene_ranges$intron_overlap = unlist(int_over)
# hits = findOverlaps(gene_ranges, intron_boundaries)
# overlaps = pintersect(gene_ranges[queryHits(hits)], intron_boundaries[subjectHits(hits)])
# gene_ranges$intron_overlap = width(overlaps) / width(intron_boundaries[subjectHits(hits)])
...
# # y = exon_boundaries
# exon_boundaries = makeGRangesFromDataFrame(gff[which(gff$feature == 'CDS'),])
# ex_over = pbmclapply(1:length(gene_ranges), function(x) {wmeanrho_overlap(gene_ranges[x], exon_boundaries)})
...
# intron_boundaries = makeGRangesFromDataFrame(gff[which(gff$feature == 'intron'),])
# int_over = pbmclapply(1:length(gene_ranges), function(x) {wmeanrho_overlap(gene_ranges[x], intron_boundaries)})
...
if (chromosome == "all") {
  save(df_pos, file = paste("Output/RhoGradient_ATG_", dataset, ".Rda", sep = ""))
} else {
  save(df_pos, file = paste("Output/RhoGradient_ATG_", dataset, "_", chromosome, ".Rda", sep = ""))
}

...
# 
# 
# write.csv(g, "Data/Recombination/LD/ldhat/", dataset, "_", chr_list, "_mask.csv")
# 
# 
# msk_kb = 20
...
genes = genes[unlist(pbmclapply(genes,
                                function(x) {"CDS" %in% x$feature},
                                mc.cores = ncores))]

# For each gene, get a range and a random control ----

cat("Get position of TTS\n")
gene_tts = pbmclapply(genes,
...
make.range = function(x) {
  ...
  if ("+" == unique(as.character(strand[x]))) {
    start = as.numeric(gene_tts[x]) + as.numeric(idx)
    ...
    start = as.numeric(gene_tts[x]) - abs(interval) + 1 - as.numeric(idx)
    end = as.numeric(gene_tts[x]) - as.numeric(idx)
    range = GRanges(seqnames = as.character(seqname[x]),
...
#   make.range(i)
# }
gene_ranges = pbmclapply(1:length(gene_tts),
...
if (chromosome == "all") {
  save(df_pos, file = paste("Output/RhoGradient_5kbTTS_", dataset, ".Rda", sep = ""))
} else {
  save(df_pos, file = paste("Output/RhoGradient_5kbTTS_", dataset, "_", chromosome, ".Rda", sep = ""))
}

...
p1 = ggplot(data = df_meanrho[which(df_meanrho$Category %in% c("Genic", "Intergenic", "Intergenic (buffer)")),],
            aes(x = species_label, y = median, fill = Category)) +
  geom_bar(position = "dodge", stat = "identity") +
  geom_errorbar(aes(x = species_label, ymin = median_lower_95, ymax = median_upper_95, fill = Category),
                position=position_dodge(width=0.9)) +
  # scale_fill_manual(values = brewer.pal(5, "Dark2")[1:3]) +
  scale_fill_grey(labels = c("Genic", "Intergenic", "Intergenic (buffer)    ")) +
  xlab("Species") + ylab("Median ρ/kb") +
...
p2 = ggplot(data = df_meanrho[which(!(df_meanrho$Category %in% c("Genic", "Intergenic", "Intergenic (buffer)"))),],
            aes(x = feature, y = median, fill = Category)) +
  geom_bar(stat="identity") +
  geom_errorbar(aes(x = feature, ymin = median_lower_95, ymax = median_upper_95)) +
  # scale_fill_manual(values = brewer.pal(5, "Dark2")[4:5]) +
  scale_fill_grey() +
  xlab("") + ylab("Median ρ/kb") +
...
# ggsave(file = paste("Figure/Paper/Fig2.jpeg", sep = ""), width = 16, height = 10, dpi = 300)

ggsave(file = paste("Figure/Paper/Fig2.jpeg", sep = ""), width = 16, height = 10, dpi = 600)
